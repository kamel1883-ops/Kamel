...
export default function Licenses() {
  ...
  const isAr = lang === "ar";
  const t = isAr ? {
    title: "تراخيص المنشأة", subtitle: "متابعة تراخيص الجهات الحكومية وعقود الصيانة وتواريخ انتهائها", add: "إضافة ترخيص",
    ...
    title: "Organization licenses", subtitle: "Track government licenses and maintenance contracts and their expiry dates", add: "Add license",
  ...
  const stats = { active: 0, expiring_soon: 0, expired: 0, not_applicable: 0, filled: 0 };
  LICENSE_TYPES.forEach((tt) => {
  ...
  return (
    <div dir={isAr ? "rtl" : "ltr"}>
      <PageHeader title={t.title} subtitle={t.subtitle} action={<Button onClick={() => { setEditing(null); setFixedType(null); setFormOpen(true); }} className="gap-2"><Plus size={18} /> {t.add}</Button>} />
      ...
      {loading ? (
        ...
        <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-4">
          {LICENSE_TYPES.map((tt) => {
            const rec = recOf(tt.key);
            return <Slot key={tt.key} type={tt} rec={rec} onAdd={() => openNew(tt.key)} onEdit={() => openEdit(rec)} t={t} />;
          })}
        </div>
      )}
...
function Slot({ type, rec, onAdd, onEdit, t }) {
    ...
    <div className={cn("bg-white rounded-2xl border p-5 flex flex-col gap-3 transition-shadow", status === "expired" ? "border-rose-200 shadow-rose-50" : status === "expiring_soon" ? "border-amber-200 shadow-amber-50" : status === "active" ? "border-emerald-100" : "border-border")}>
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2">
          <div className={cn("w-9 h-9 rounded-xl flex items-center justify-center", na ? "bg-slate-100 text-slate-400" : "bg-slate-900 text-white")}><ShieldCheck size={18} /></div>
