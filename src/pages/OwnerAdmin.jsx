...
function RelinkEmailDialog({ open, onClose, tenant, isAr, onSaved }) {
  ...
  const f = isAr ? {
    ...
    note: "استخدم هذا الإجراء عندما يتواصل معك العميل بعد نسيان بريده القديم تماماً. سيتم استبدال بريد المنشأة بالإيميل الجديد وإرسال رسالة تفعيل إلى بريده الجديد توجّهه لاستخدام «نسيت كلمة المرور» بالإيميل الجديد + الرقم الوطني الموحد للمنشآت. لا تُرسل له رسالة تسجيل جديد — فقط رسالة التفعيل. بيانات المنشأة (الموظفون، الرواتب، الحقوق، المركبات، الطلبات...) محفوظة بالكامل ومرتبطة بالرقم الوطني الموحّد للمنشآت الثابت لا بالإيميل، فلا تُفقد شيئاً عند تغيير البريد.",
    cancel: "إغلاق", confirm: "ربط الإيميل الجديد",
    invalid: "بريد إلكتروني غير صالح",
    sameErr: "البريد الجديد مطابق للبريد الحالي",
    ok: (em) => `تم استبدال بريد المنشأة بـ«${em}» وإرسال رسالة التفعيل إلى بريده الجديد. رسالة التفعيل توجّهه لفتح «نسيت كلمة المرور» وكتابة الإيميل الجديد + الرقم الوطني الموحّد للمنشآت لاستلام رابط تعيين كلمة المرور، ثم الدخول بالإيميل الجديد + الرقم الوطني الموحّد للمنشآت + كلمة المرور الجديدة. بيانات المنشأة محفوظة بالكامل ومرتبطة بالرقم الوطني الموحّد للمنشآت الثابت، ولم يُرسل له رابط تسجيل جديد.`,
    ...
    note: "Use this when a client has completely forgotten their old email. The company email will be updated and an invite to set up an account will be sent to the new one. The client can then sign in via the company portal with the new email + National Unified Number + password, or use “Forgot password” to receive a setup link at the new email.",
    cancel: "Close", confirm: "Link new email",
    invalid: "Invalid email address",
    sameErr: "New email is the same as the current one",
    ok: (em) => `Linked “${em}” to the organization and sent a setup invite to it. Ask the client to complete the invite (or use “Forgot password”), then sign in with the new email + National Unified Number + password.`,
  ...
  const submit = async () => {
    ...
    try {
      ...
      try {
        await base44.functions.invoke("sendRelinkActivation", {
          new_email: em,
          unified_number: String(tenant?.unified_number || ""),
          client_name: tenant?.name || "",
        });
      } catch (e) {
        // إرسال رسالة التفعيل — لا يوقف العملية إن فشل.
      }
      setDone(f.ok(em));
      onSaved?.();
    } catch (e) {
