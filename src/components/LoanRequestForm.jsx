      ...
      <DialogContent className="max-w-md">
        ...
        <form onSubmit={submit} className="space-y-4">
          <div className="text-sm text-muted-foreground">
            الموظف: <b className="text-foreground">{employee?.employee_number} - {employee?.position}</b>
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs font-medium text-muted-foreground">مبلغ السلفة (ر.س)</Label>
            <Input type="number" min={0} value={form.amount} onChange={(e) => set("amount", e.target.value)} required />
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs font-medium text-muted-foreground">عدد الأقساط الشهرية</Label>
          ...
          {amount > 0 && (
            <div className="text-xs text-muted-foreground bg-slate-50 rounded-lg p-3">
              سيُخصم قسط شهري قدره <b className="text-foreground">{monthly} ر.س</b> من الراتب على مدى {installments} شهر.
            </div>
          )}
          <div className="space-y-1.5">
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={saving || amount <= 0}>
              {saving && <Loader2 size={16} className="animate-spin ml-2" />} تقديم الطلب
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
