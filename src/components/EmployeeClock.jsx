...
const getPosition = () =>
  new Promise((resolve, reject) => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => reject(new Error("تعذر الوصول إلى موقعك — فعّل صلاحية الموقع")),
...
export default function EmployeeClock({ employee, org, onChanged }) {
  ...
  const loadToday = async () => {
    ...
    try {
      const recs = await base44.entities.Attendance.filter(
        { employee_id: employee.id, date: localToday() }, "-created_date", 5
      );
  ...
  const hasWorkplace =
    org && org.workplace_lat != null && org.workplace_lat !== "" &&
    org.workplace_lng != null && org.workplace_lng !== "";

  const run = async (kind) => {
    setMsg({ type: "", text: "" });
    if (!hasWorkplace) {
      setMsg({ type: "err", text: "لم يحدد المقر الرسمي — تواصل مع الموارد البشرية" });
      return;
    }
    setBusy(true);
    try {
      ...
      if (dist > radius) {
        setMsg({ type: "err", text: `أنت خارج نطاق العمل (المسافة ${Math.round(dist)} متر). يُسمح بالبصمة ضمن ${radius} متر فقط.` });
        setBusy(false);
        return;
      }
      if (kind === "in") {
        if (today && today.check_in) {
          setMsg({ type: "err", text: "تم تسجيل الحضور مسبقاً اليوم" });
          setBusy(false);
          return;
        }
        const name = `${employee.employee_number} - ${employee.position}`;
        if (today) {
          await base44.entities.Attendance.update(today.id, {
            check_in: nowHM(), status: "present", source: "portal", employee_user_id: employee.user_id,
          });
        } else {
        ...
        setMsg({ type: "ok", text: "تم تسجيل الحضور بنجاح" });
      } else {
        if (!today || !today.check_in) {
          setMsg({ type: "err", text: "يجب تسجيل الحضور أولاً" });
          setBusy(false);
          return;
        }
        if (today.check_out) {
          setMsg({ type: "err", text: "تم تسجيل الانصراف مسبقاً" });
          setBusy(false);
          return;
        }
        ...
        await base44.entities.Attendance.update(today.id, {
          check_out: nowHM(), work_hours: wh, source: "portal",
        });
        setMsg({ type: "ok", text: "تم تسجيل الانصراف بنجاح" });
      }
      await loadToday();
      onChanged?.();
    } catch (e) {
      setMsg({ type: "err", text: e?.message || "تعذر التسجيل" });
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="bg-white rounded-2xl border border-border p-5 mb-6">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h3 className="font-semibold text-sm">البصمة اليومية</h3>
          <p className="text-xs text-muted-foreground">يُسمح بالبصمة فقط من مقر العمل ضمن {radius} متر — بتاريخ {localToday()}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        ...
        <div className="rounded-xl bg-blue-50/60 border border-blue-100 p-3">
          <div className="text-xs text-muted-foreground">الانصراف</div>
      ...
      <div className="flex flex-wrap gap-2">
        {!checkedOut ? (
          ...
          <div className="flex items-center gap-2 text-emerald-700 text-sm font-medium">
            <CheckCircle2 size={18} /> اكتمل تسجيل اليوم ({today?.work_hours || 0} ساعة)
          </div>
        )}
      </div>
      ...
      {!hasWorkplace && (
        <div className="mt-3 flex items-center gap-2 text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
          <MapPin size={14} /> لم يحدد المقر الرسمي بعد.
        </div>
      )}
      {msg.text && (
