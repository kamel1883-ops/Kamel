...
export default function EmployeeDecisions({ items, session, onReload }) {
  const { lang } = usePortalI18n();
  const t = usePortalT("decisions");
  ...
  const ack = async (r) => {
    setBusy(r.id);
    try {
      await base44.functions.invoke("portalData", { token: session.token, employee_id: empId, action: "ack_decision", id: r.id });
      ...
      {items.length === 0 ? (
        ...
        <div className="space-y-2.5">
          {items.map((r) => {
            ...
            return (
              <div key={r.id} className="rounded-xl border border-border bg-muted/30 overflow-hidden">
                <button type="button" onClick={() => setExpanded(open ? null : r.id)} className="w-full text-right p-3">
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-xs px-2 py-0.5 rounded-full bg-blue-50 text-blue-600 shrink-0">{t.types[r.decision_type] || r.decision_type}</span>
                    <span className="text-xs text-muted-foreground me-auto" dir="ltr">{t.issued}: {r.issued_date || "—"}</span>
                  ...
                  {r.effective_date && <div className="text-[11px] text-muted-foreground mt-1">{t.effective}: {r.effective_date}</div>}
                </button>
                {!acked && (
                  <div className="px-3 pb-3">
                    <button onClick={() => ack(r)} disabled={busy === r.id} className="inline-flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg bg-violet-600 text-white hover:bg-violet-700 disabled:opacity-50">
                      {busy === r.id ? <Loader2 size={13} className="animate-spin" /> : <CheckCircle2 size={13} />} {t.ack}
                    </button>
                  </div>
                )}
              </div>
