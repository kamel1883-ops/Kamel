...
export function ClientInfoDialog({ open, onClose, tenant, isAr, t, onAction, busyId }) {
  ...
  const rows = [
    ...
    [isAr ? "عدد الموظفين (المعلن)" : "Declared employees", tenant.employee_count],
    [isAr ? "الموظفون النشطون فعلياً" : "Active employees (actual)", tenant.employees_active_count ?? 0],
    [isAr ? "إجمالي الموظفين المسجلين" : "Total employees (actual)", tenant.employees_total_count ?? 0],
